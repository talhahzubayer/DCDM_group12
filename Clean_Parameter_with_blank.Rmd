---
title: "Clean_parameter_description"
author: "Mia"
date: "2025-11-17"
...
getwd()
parameter_doc <- read.csv(
  "Desktop/Group12/IMPC_parameter_description.csv",
  header = TRUE,
  strip.white = TRUE
)
readLines("Desktop/Group12/IMPC_parameter_description.csv", n = 5)
```
...
change the blank row to NA 
```{r}
final_clean_data[] <- lapply(final_clean_data, function(x) {
  if(is.factor(x)) x <- as.character(x)   # convert factors to characters
  x[x == ""] <- NA                        # replace blank with NA
  return(x)
})
